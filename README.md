...
- Los formularios se guardan en la colección `formularios` de Firestore.
- La app usa autenticación anónima para asociar cada registro al usuario actual (`userId`).
- El listado de formularios se carga desde Firestore al iniciar sesión.
- Guardar/editar y eliminar se ejecuta directamente contra la base de datos en la nube.
